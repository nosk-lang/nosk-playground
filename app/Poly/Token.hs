  ...
  P.isEndOfInput >>= \case
    ...
    False -> Latin.any () >>= \case
      ...
      '>' -> P.peek' () >>= \case
        ...
        _ -> P.effect (Builder.push AngleClose b0) >>= step
      '=' -> P.effect (Builder.push Equals b0) >>= step
...
termChar :: Word8 -> Bool
termChar w = let c = word8ToChar w in
  (c >= 'a' && c <= 'z') ||
  (c >= 'A' && c <= 'Z') ||
  (c >= '0' && c <= '9')
